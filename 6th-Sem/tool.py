...
def helper():
        ...
        print("------------")
        print("\t./tool.py example.com: Scans the domain example.com.")
        print("\t./tool.py example.com --skip dmitry --skip theHarvester: Skip the 'dmitry' and 'theHarvester' tests.")
        print("\t./tool.py example.com --nospinner: Disable the idle loader/spinner.")
        print("\t./tool.py --update   : Updates the scanner to the latest version.")
        print("\t./tool.py --help     : Displays this help context.")
        print(bcolors.OKBLUE+"Interactive:"+bcolors.ENDC)
        print("------------")
        print("\tCtrl+C: Skips current test.")
        print("\tCtrl+Z: Quits UIT Scanner.")
...
def logo():
  ...
  / / / /  _/_  __/  / ___/_________ _____  ____  ___  _____
 / / / // /  / /     \__ \/ ___/ __ `/ __ \/ __ \/ _ \/ ___/
/ /_/ // /  / /     ___/ / /__/ /_/ / / / / / / /  __/ /    
\____/___/ /_/     /____/\___/\__,_/_/ /_/_/ /_/\___/_/
"""
    print(logo_ascii)
...
class Spinner:
    ...
    def spinner_task(self):
        ...
        try:
            ...
            print("\n\t"+ bcolors.BG_ERR_TXT+"UIT Scanner received a series of Ctrl+C hits. Quitting..." +bcolors.ENDC)
            sys.exit(1)
...
tool_cmd   = [
                ...
                #2
                ["wget -O /tmp/uit_temp_aspnet_config_err --tries=1 ","/%7C~.aspx"],

                #3
                ["wget -O /tmp/uit_temp_wp_check --tries=1 ","/wp-admin"],

                #4
                ["wget -O /tmp/uit_temp_drp_check --tries=1 ","/user"],

                #5
                ["wget -O /tmp/uit_temp_joom_check --tries=1 ","/administrator"],
                ...
                #75
                ["wget -O /tmp/uit_temp_aspnet_elmah_axd --tries=1 ","/elmah.axd"],
                ...
                #78
                ["wapiti "," -f txt -o uit_temp_wapiti"],
...
tools_fix = [
                    ...
                    [30, "Particular Scanner found multiple vulnerabilities that an attacker may try to exploit the target.",
                            "Refer to uit-Vulnerability-Report to view the complete information of the vulnerability, once the scan gets completed."],
...
if args_namespace.help or (not args_namespace.update \
    ...
    logo()
    print("UIT Scanner is updating....Please wait.\n")
    spinner.start()
    ...
    if oldversion_hash == newversion_hash :
        clear()
        print("\t"+ bcolors.OKBLUE +"You already have the latest version of UIT Scanner Tool." + bcolors.ENDC)
    else:
        clear()
        print("\t"+ bcolors.OKGREEN +"UIT Scanner successfully updated to the latest version." +bcolors.ENDC)
    spinner.stop()
    sys.exit(1)
    ...
    while (rs_avail_tools < len(tools_precheck)):
        ...
        try:
            ...
            print("\t"+bcolors.BG_ERR_TXT+"UIT Scanner was terminated abruptly..."+bcolors.ENDC)
            sys.exit(1)
    ...
    if len(unavail_tools_names) == 0:
        print("\t"+bcolors.OKGREEN+"All Scanning Tools are available. Complete vulnerability checks will be performed by UIT Scanner."+bcolors.ENDC)
    else:
        print("\t"+bcolors.WARNING+"Some of these tools "+bcolors.BADFAIL+str(unavail_tools_names)+bcolors.ENDC+bcolors.WARNING+" are unavailable or will be skipped. UIT Scanner will still perform the rest of the tests. Install these tools to fully utilize the functionality of UIT Scanner."+bcolors.ENDC)
    ...
    while(tool < len(tool_names)):
        ...
        temp_file = "/tmp/uit_temp_"+tool_names[tool][arg1]
        ...
        if runTest == 1:
                ...
                print("\n"+bcolors.WARNING + "\tTest Skipped. Performing Next. Press Ctrl+Z to Quit UIT-Scanner.\n" + bcolors.ENDC)
                rs_skipped_checks = rs_skipped_checks + 1
    ...
    if len(rs_vul_list)==0:
        ...
        with open("uit-Vulnerability-Report", "a") as report:
            while(rs_vul < len(rs_vul_list)):
                vuln_info = rs_vul_list[rs_vul].split('*')
                report.write(vuln_info[arg2])
                report.write("\n------------------------\n\n")
                temp_report_name = "/tmp/uit_temp_"+vuln_info[arg1]
            ...
            print("\tComplete Vulnerability Report for "+bcolors.OKBLUE+target+bcolors.ENDC+" named "+bcolors.OKGREEN+"`uit-Vulnerability-Report`"+bcolors.ENDC+" is available under the same directory UIT Scanner resides.")
    ...
    for file_index, file_name in enumerate(tool_names):
        with open("uit-Debug-ScanLog", "a") as report:
            try:
                with open("/tmp/uit_temp_"+file_name[arg1], 'r') as temp_report:
                        data = temp_report.read()
    ...
    print("\n")
    print("\tFor Debugging Purposes, You can view the complete output generated by all the tools named "+bcolors.OKBLUE+"`UIT-Debug-ScanLog`"+bcolors.ENDC+" under the same directory.")
    print(bcolors.BG_ENDL_TXT+"[ Report Generation Phase Completed. ]"+bcolors.ENDC)

    os.system('setterm -cursor on')
    os.system('rm /tmp/uit_te* > /dev/null 2>&1') # Clearing previous scan files
